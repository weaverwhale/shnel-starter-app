...
import { fetchData, fetchInsights } from './data.ts';
  ...
  const loadData = async (): Promise<void> => {
    try {
      ...
      if (fetchedData && fetchedData.data && fetchedData.data.length > 0) {
        ...
        try {
          const insightsPrompt = "Analyze this data and provide key business insights, trends, and recommendations. Focus on sales performance, growth patterns, and any notable observations.";
          const { insights } = await fetchInsights(fetchedData, insightsPrompt);
          setInsights(insights);
          console.log('insights from API', insights);
        } catch (insightsErr) {
                ...
                <div className="bg-white/90 p-6 rounded-lg shadow-sm">
                  <div className="markdown-content">
                    <ReactMarkdown 
                      ...
                      components={{
                        h1: ({node, ...props}) => <h1 className="text-3xl font-bold text-gray-900 mt-8 mb-4 pb-3 border-b-2 border-gray-300 first:mt-0" {...props} />,
                        h2: ({node, ...props}) => <h2 className="text-2xl font-bold text-gray-900 mt-8 mb-4 pb-2 border-b border-gray-300 first:mt-0" {...props} />,
                        h3: ({node, ...props}) => <h3 className="text-xl font-bold text-indigo-800 mt-6 mb-3 first:mt-0" {...props} />,
                        h4: ({node, ...props}) => <h4 className="text-lg font-semibold text-gray-800 mt-5 mb-2 first:mt-0" {...props} />,
                        p: ({node, ...props}) => <p className="text-gray-800 my-4 leading-relaxed first:mt-0" {...props} />,
                        a: ({node, ...props}) => <a className="text-indigo-600 underline font-medium hover:text-indigo-800 transition-colors" {...props} />,
                        ul: ({node, ...props}) => <ul className="my-5 pl-6 space-y-2 list-disc" {...props} />,
                        ol: ({node, ...props}) => <ol className="my-5 pl-6 space-y-2 list-decimal" {...props} />,
                        ...
                        table: ({node, ...props}) => <div className="my-8 overflow-x-auto"><table className="w-full border-collapse shadow-sm rounded-lg overflow-hidden" {...props} /></div>,
                        thead: ({node, ...props}) => <thead className="bg-indigo-50 border-b-2 border-indigo-200" {...props} />,
                        tbody: ({node, ...props}) => <tbody {...props} />,
                        tr: ({node, ...props}) => <tr className="border-b border-gray-200 hover:bg-gray-50 transition-colors" {...props} />,
                        th: ({node, ...props}) => <th className="px-4 py-3 text-left font-bold text-gray-900 border border-gray-300" {...props} />,
                        td: ({node, ...props}) => <td className="px-4 py-3 border border-gray-200 text-gray-800" {...props} />,
                        code: ({node, ...props}) => {
                          const isInline = !(props as any).className?.includes('language-');
                          return isInline 
                            ? <code className="bg-indigo-50 px-2 py-0.5 rounded text-sm text-indigo-700 font-mono font-semibold border border-indigo-200" {...props} />
                            : <code className="block bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto my-5 text-sm" {...props} />;
                        },
                        pre: ({node, ...props}) => <pre className="bg-gray-900 text-gray-100 p-5 rounded-lg overflow-x-auto my-6 shadow-lg" {...props} />,
                        blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-indigo-500 pl-6 pr-4 py-3 my-6 italic text-gray-700 bg-indigo-50/50 rounded-r-lg" {...props} />,
